...
# Timezone Indonesia Barat (UTC+7)
...
def get_utc_timestamps_for_day(target_date):
    """
    Calculates the 24-hour window:
    from 21:00 UTC (H-1) to 20:59:59 UTC (hari target)
    """
...
def get_api_data(url, params):
    ...
    try:
        ...
        log_update(f"API request failed for {url} | params={params} | error={err}")
        return None
...
def transform_details_to_json(details, match_id):
    """Transforms API response into final JSON format (WIB)."""
    if not details:
        return None
    ...
    return {
        ...
        "time": wib_dt.strftime('%H:%M'),
        "date": wib_dt.strftime('%d-%m-%Y'),
        "timezone": "WIB",
        "links": links
    }
...
def main():
    ...
    log_update(
        f"Scanning matches window ending {target_date.strftime('%Y-%m-%d')} 21:00 UTC"
    )
    ...
    # Sort by date + time (WIB)
    all_formatted_matches.sort(
        key=lambda x: (
    ...
    with open(OUTPUT_FILENAME, 'w', encoding='utf-8') as f:
        json.dump(all_formatted_matches, f, indent=4, ensure_ascii=False)
...
if __name__ == "__main__":
    main()        for source in alternative_streams:
            if source.get('path') and source['path'].startswith('http'):
                links.append(source['path'])
