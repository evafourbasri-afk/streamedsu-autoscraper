...
PLAYLIST_FILE = "ppvgit.m3u"   # ← HASIL M3U (baru)
...
async def main():
    ...
    # FLATTEN STREAMS + REMOVE 24/7
    for cat_obj in categories:
        original_cat = cat_obj.get("category", "").strip()

        # ❌ Jangan tampilkan kategori 24/7
        if original_cat.lower() == "24/7 streams":
            continue
        ...
        for idx, s in enumerate(streams, start=1):
            ...
            if urls:
                ...
                valid_streams.append({
                    ...
                    "ends_at": s["ends"]["ends_at"] if isinstance(s.get("ends_at"), dict) else s.get("ends_at"),
                    "url": found,
                    "time": s["clock_time"]
                })
            else:
